...
const char version_of_program[] = "message2action version 0.39\n";
...
if (configfilep != 0) { // opening of configfile was successfull
	...
	if (fileattributes.st_size > MAX_CONFIG_FILE_SIZE) 
		{ 	// filesize of config-file is greater than specified, we stop here
			...
			while (configparameters[configparametercount++] != NULL); // look for NULL-pointer at end of array in order to determine the # of parameters we have to search for. 
...
char tempchar[5*MAX_CHARS_FOR_PATH] = "";
int tempint = 0;
...
char logentry[MAX_CHARS_PER_LINE_LOGFILE + 100]; 		// one line for the logfile
char message_content[MAX_SIZE_CONFIG_TEXT + 250] = "";		// the content for the message (mail, sms, telegram ...) we will create
char fullpath_for_message[MAX_CHARS_PER_LINE_CONFIGFILE];
char tempstring[200] = "";
int randomnumber;
...
char date[11] = "\0";		// current date
char garbage_collection_date[11] = "\0";	// date of last garbage collection
...
int tempint;
char logentry[MAX_CHARS_PER_LINE_LOGFILE + 200] = "";
...
//*****************************************************************************************************************
// stop_processing_message: stops processing incoming messages. It simply creates a message which contains the command for stopping. 
...
char temp_line[MAX_CHARS_PER_LINE_LOGFILE] = "";
char logentry[MAX_CHARS_PER_LINE_LOGFILE + 200] = "";
char telegram_chat_id[80] = ""; 
char mail_address[40] = "";
char cell_phone[20] = "";
FILE *processidfilep;
int processid = 0;
...
if ( stopfile_flag == STOPFILE_TRUE ) {
	...
	if ( filep != 0 ) {
		return_cellphonenumber_from_inifile(cell_phone);	// we get first valid cell-phone-# from ini-file
		sprintf(temp_line, "From: %s", cell_phone);
		#ifdef DEVSTAGE
...
if ( stopfile_flag == STOPFILE_TRUE ) {
	...
	filep = fopen(stopfile, "w+");
	if ( filep != 0 ) {
		return_mailaddress_from_inifile(mail_address);	// we get first valid mailaddress from ini-file
		sprintf(temp_line, "From: <%s>", mail_address);
		#ifdef DEVSTAGE
...
if ( stopfile_flag == STOPFILE_TRUE ) {
	...
	if ( filep != 0 ) {		
		...
		return_chatid_from_inifile(telegram_chat_id);	// we get first valid chat-id from ini-file
		// the following string is constant, none of the values will be read:
		sprintf(temp_line, "%s\nMessage: %s", telegram_chat_id, configvariables[message2action_stop_command_count]); 
		fputs(temp_line, filep); 				
		fclose(filep);
